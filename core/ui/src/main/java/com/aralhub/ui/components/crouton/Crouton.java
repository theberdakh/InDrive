...
public final class Crouton {
    ...
    private void measureCroutonView() {
        View view = getView();
        int widthSpec;
        widthSpec = View.MeasureSpec.makeMeasureSpec(Objects.requireNonNullElseGet(viewGroup, () -> activity.getWindow().getDecorView()).getMeasuredWidth(), View.MeasureSpec.AT_MOST);
    ...
    private RelativeLayout initializeContentView(final Resources resources) {
        ...
        //top padding is multiplied to be visible below camera
        contentView.setPadding(padding, padding * 5, padding, 0);
        ...
        RelativeLayout.LayoutParams textParams = getTextParams(image);

        contentView.addView(text, textParams);
        return contentView;
    }

    private RelativeLayout.LayoutParams getTextParams(ImageView image) {
